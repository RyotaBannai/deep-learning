...
from src.part2.candicate_seg_model import SegmentationAugmentation, UNetWrapper
from src.part2.is_nodule_clasif_model import LunaModel
...
class LunaTrainingApp:
    ...
    def initModel(self):
        ...
        if self.use_mps:
            ...
            # if torch.backends.mps.device_count() > 1:
        ...
        for epoch_ndx in range(1, self.cli_args.epochs + 1):
            ...
            log.info(
                "Epoch {} of {}, {}/{} batches of size {}*{}".format(
                    ...
                    # (torch.backends.mps.device_count() if self.use_mps else 1),
                    1,
                )
            )
    ...
    def logMetrics(
        ...
        metrics_dict["correct/all"] = (
            (truePos_count + trueNeg_count) / np.float32(metrics_t.shape[1]) * 100
        )
        metrics_dict["correct/neg"] = trueNeg_count / np.float32(neg_count) * 100
        metrics_dict["correct/pos"] = truePos_count / np.float32(pos_count) * 100
        ...
        f1 = metrics_dict["pr/f1"] = 2 * (precision * recall) / (precision + recall)

        # keyward 引数だから ** で辞書を展開するとよしなにlogging できる
        log.info(
            (
                "E{} {:8} {loss/all:.4f} loss, "
                + "{correct/all:-5.1f}% correct, "
                + "{pr/precision:-5.1f}% correct,"
                + "{pr/recall:-5.1f}% correct,"
            ).format(
                epoch_ndx,
                mode_str,
                **metrics_dict,
