...
from src.utils.disk import getCache  # noqa: E402
from src.utils.logconf import logging  # noqa: E402
from src.utils.util import XyzTuple, xyz2irc  # noqa: E402
...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob(str(data_dir / "subset*/*.mhd"))
...
class Ct:
    def __init__(self, series_uid):
        # print(f"{series_uid=}")
        mhd_path = glob.glob(str(data_dir / f"subset*/{series_uid}.mhd"))[0]
...
def getCtRawCandidate(series_uid, center_xyz, width_irc):
    ...
    # print(width_irc)
    # exit(0)
    # series_uid = "1.3.6.1.4.1.14519.5.2.1.6279.6001.137763212752154081977261297097"
    # center_xyz = (-58.7863496368, 116.513803495, -476.010277346)
    # width_irc = (32, 48, 48)
    ct = getCt(series_uid)
...
def getCtAugmentedCandidate(augmentation_dict, series_uid, center_xyz, width_irc, use_cache=True):
    ...
    for i in range(3):
        ...
        if "offset" in augmentation_dict:
            offset_float = augmentation_dict["offset"]  # 固定値で 0.1 としてる
            # random()
            # この関数はランダムな浮動小数点数を半開区間 0.0 <= X < 1.0 内に一様に生成
            random_float = random.random() * 2 - 1  # -1<=X<1 の区間の値に変換
    ...
    if "rotate" in augmentation_dict:
        ...
        # 回転はxy 平面に限定
        # z 軸方向には、立方体ではなく、それらの軸を交換可能なものとして考えることができない
